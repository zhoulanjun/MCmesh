...
+ (MBProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:view offSet:0];
    [view addSubview:hud];
    [hud show:animated];
#if __has_feature(objc_arc)
    return hud;
...
- (id)initWithWindow:(UIWindow *)window {
    return [self initWithView:window offSet:0];
}

- (id)initWithView:(UIView *)view offSet:(NSInteger) offset {
...
- (void)layoutSubviews {
    ...
    indFrame.origin.y = floorf((frame.size.height - indFrame.size.height) / 2) + self.yOffset+self.offset;
...
- (void)drawRect:(CGRect)rect {
    ...
    CGRect boxRect = CGRectMake(roundf((allRect.size.width - self.width) / 2) + self.xOffset,
                                roundf((allRect.size.height - self.height) / 2) + self.yOffset+self.offset, self.width, self.height);
    // Corner radius
    float radius = 10.0f;
